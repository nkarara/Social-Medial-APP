...
const FollowingPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchFollowing = async () => {
      setLoading(true);
      try {
        // Simulated following data until we have a real follows table
        ...
        // If user is logged in, set follow status for each followed user
        if (user) {
          const status: Record<string, boolean> = {};
          for (const followed of data || []) {
            status[followed.id] = true; // All are being followed in this view
          }
